...
Docs Hygiene reads `docs-hygiene.yml` from the checked project root unless `--config` points to another file. Breaking field renames are listed in [Terminology Migration](../migration/terminology.md).
...
defined in [Slug Identity Governance](../capabilities/slug-identity.md).
...
For this layout, `docs/engineering/01_language.md` should have
`docs/zh/engineering/01_language.md`.
...
`documentContracts.profiles` infers a document type from path and file name; `documentContracts.templates` supplies reusable contract fragments through stable profile bindings. The first matching profile applies. See [Document Contracts](../governance/document-contracts.md) and the opt-in [Document Kind Registry](../capabilities/document-kinds.md).
...
Multidimensional `hygieneProfile.dimensions` configuration, migration, evidence, output, and CI semantics are defined in [Hygiene Profiles](../governance/hygiene-profile.md).
...
or error states. See [Progressive Rule Activation](../governance/progressive-rule-activation.md) for details.

## Governance Graph

`governance.manifests` enables the graph; `governance.topology` reports deterministic bridge-connected communities and optionally enforces Fan/cycle thresholds, a `communityBaseline`, and exact audited supernode exceptions; `governance.ownership` enables the offline principal directory, established-identity Owner/review/knowledge obligations, and atomic review-reset audit; `governance.contentAnchors.verifyGitCommits` opts into local Git verification for explicit repo anchors and defaults to `false`. File and block SHA-256 anchors do not require it. `governance.coreClaims` adds explicit Library SSOT policy, `governance.criticalDependencies` selects edges that must be pinned, and `governance.portableSnapshots` registers offline commit evidence and optional Ed25519 trust. See [Governance Graph](../governance/governance-graph.md), [Library Claim Governance](../capabilities/library-claims.md), [Critical Dependency Pins](../capabilities/critical-dependency-pins.md), [Portable Commit Snapshots](../capabilities/portable-snapshots.md), [Audited Supernode Exceptions](../capabilities/supernode-exceptions.md), and [Document Ownership](../capabilities/document-ownership.md).
