...
每项声明提供稳定 `id`、语义 `version`、`refinementLevel`、`referenceRelation`
和生命周期 `status`。精化层级可取 `intent`、`definition` 和 `implementation`；
...
每个 Body 至少声明一条指向同一精化层级 Library 的 `references` 边：

- Intent Body 指向 UL；
- Definition Body 指向 Glossary；
- Implementation Body 指向 SDK。

目标缺失、目标为 Body 或目标位于其他精化层级都是错误。Library 不能使用 `references`
代替 `projects`。
...
跳过中间精化层级、反转边方向、使用错误引用关系或无法解析 `id@version` 都是错误。
水平边不能满足缺失的垂直边。
...
当前关系图校验资产级身份、版本、精化层级、引用关系、边类型、可达性、递归
Package 成员和语言表示身份一致性。
