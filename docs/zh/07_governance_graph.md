...
内容锚、生命周期出处、类型化 expectation、候选端点和显式 `resolved`、`unresolved`、
`ambiguous` 或 `incompatible` 结果。画像报告有序节点与边，并给出解析、关系类型和
孤立节点指标；过期检测、反向完整性、影响与拓扑消费这些记录。标准 Markdown Link 仍保持
...
规范化器处理。Markdown 默认不进入语义图因此成为可检查策略。occurrence 契约保持
稳定；规范化 edge JSON 按 C-020 增量增加类型化 expectation 与 resolution。详见
...
`file` 或 `repo` scope。file 对完整目标做 hash；block 要求标题 slug `locator`，只对
该 ATX 标题段做 hash。多个项目形成独立排序的锁定边，并在各自声明行报告失败。

repo scope 使用 `algorithm: git` 和完整 commit OID，默认关闭；只有配置
`governance.contentAnchors.verifyGitCommits: true` 后，检查器才比较该 commit 与当前完整
tracked repository state；untracked 文件不影响结果。既有内联 selector 加 SHA-256 的链接继续锁定整文件。详见
...
关系图还报告确定性的桥连通社区、有序成员和不同的有向跨社区边。重复链接和并行关系
不改变成员。`communityBaseline` 记录期望成员；除非设置
`enforceCommunityBaseline: true`，变化只报告、不阻断。
...
governance:
  topology:
    ...
    forbidCycles: true
    communityBaseline:
      SPEC-001: community:PRD-001
    enforceCommunityBaseline: false
```
