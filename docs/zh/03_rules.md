...
`DH_OWNERSHIP_001` 表示离线 Principal 目录无效，或已建立身份缺少可解析、active、已展开
的 Owner。

`DH_REVIEW_001` 表示复核证据缺失、无效或过期；`DH_REVIEW_002` 在配置的临近窗口发出 warning。

`DH_KNOWLEDGE_001` 表示来自不同 active person 的当前理解确认少于两条；group 不算个人确认。
