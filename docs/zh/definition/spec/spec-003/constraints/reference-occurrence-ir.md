...
内建锚策略按照 C-013 支持 file/block scope 的 SHA-256，以及显式启用的 repo scope
Git commit OID。

该 IR 是内部扩展契约；字段含义或兼容性变化时必须升级 schema version。既有 file 锚
payload 保持不变，多粒度锚按照 C-013 增加显式字段。规范化边遵循
[C-003](edge-normalization.md)，并按 [C-020](typed-reference-resolution.md) 增量增加
类型化 expectation 与 resolution；selector 解析遵循 [C-011](selector-resolution.md)。
