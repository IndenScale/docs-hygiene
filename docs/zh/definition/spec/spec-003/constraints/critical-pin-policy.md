...
要求分别约束允许算法、最低 scope、whole-file scope 与最大审计年龄。内容范围从小到大
统一列为 `block | file | commit`；策略强度另行明确：file 接受 block、file 或 commit，
commit 接受 block 或 commit，block 只接受 block。`DH_PIN_001` 至 `DH_PIN_006` 区分缺失、
scope 不足、算法不允许、
