...
- 边兼容、带版本的语法中立引用收集、显式 Markdown 非语义策略、语法无关规范化、
  显式 expectation/resolution 结果、确定性标题 selector、本地化签名同位和共享 slug 规范化；
- block、file、repo 锚定隔离，逐项多锚诊断，可选完整 tracked-state 校验，可移植快照导入，
  关键依赖策略，显式 Pin 更新计划，原子写入和审计输出；
- 去重的反向传递闭包、确定性循环终止、不同邻居 Fan 指标、稳定循环组、确定性桥连通
  社区、边界基线、拓扑阈值、趋势和限定作用域的超级节点例外；
