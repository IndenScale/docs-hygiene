...
这里的 Library 是角色，不限定为代码包。Intent Library 是 UL，Definition Library 是
Glossary，Implementation Library 是 SDK 及其共享类型、Schema、接口和规则。Body
是受治理主体。Library 不表示次要，Body 也不表示更加权威；区别是共享复用与具体主张。
