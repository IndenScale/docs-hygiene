...
frontmatter、语义 Wiki Link、已配置文档 Profile、受治理精化层级、显式拓扑策略和已启用
Adapter。
所有计数都先应用 ignore 策略。
...
| `governance.identity` | 身份、Package、语义引用与标题 selector |
| `governance.traceability` | 关键 Pin 与可移植证据 |
| `governance.topology` | 显式 Fan-In、Fan-Out 与循环策略 |
...
激活状态描述单条规则当前如何执行，不是文档卫生成熟度。[文档卫生治理模型](hygiene-governance-model.md)
提出在每个能力维度上独立应用基础、受控或治理成熟度，同时保留这些激活状态作为显式
执行策略。多维画像已经交付，并报告这些决策而不取代它们。
...
`governance.topology` 在 `auto` 模式下保持 inactive，直到至少配置一项显式
`governance.topology` 阈值；仅存在关系图或规模增长绝不会使其阻断。
