...
策略。未命中任何 matcher 的边仍可不带 Pin；它们可以进入结构影响分析，但上游内容变化
不会使其自动过期。Pin 会在所锁定内容哈希或等价已审议状态漂移时产生确定性失效，因此
策略只应 Pin 确实需要这种新鲜度保证的依赖。
  ...
  criticalDependencies:
    - id: reviewed-contracts
      match:
        ...
        sourcePaths: [docs/engineering/prd/**/*.md]
        targetPaths: [docs/engineering/ul/**/*.md]
        sourceIds: [PRD-1]
...
`body` 或 `library`，内建 relation 为 `references`，不受当前边是否已有 Pin 影响。
适配器特有的 Issue 关系由适配器契约治理。路径是项目相对 glob，身份列表使用稳定治理 ID。
